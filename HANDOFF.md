...
- 2026-09-21 — **`templates/` and 3 files in `docs/` moved to `andersenbuilds-brand`.** A full 9-repo
  sweep (prompted by the memory-fix video itself) found these were production assets stranded in the
  wrong repo — invisible to `andersenbuilds-brand`'s new Obsidian vault and to any session that only
  attaches that repo, which is the exact failure mode the video is about. They now live at
  `andersenbuilds-brand/templates/` and `andersenbuilds-brand/docs/`. This repo keeps `HANDOFF.md`,
  `INSTALL.md`, `docs/how-my-setup-works.md`, and `skills/project-kickoff/` — its actual job is the
  project-kickoff skill and setup notes, not brand content.
- 2026-09-20 — Added `templates/mistake-reveal.html` — the cold-open hook clip for the "I caught my AI making things up" long-form video (idea drafted this session, not yet scripted or filmed). Three claim → strike → correction beats (stack cost, brand guide, memory location — all three real mistakes from this session, caused by the same root issue: only one of nine repos attached at session start) landing on a centered payoff line, "Same repo. Every time." ~9s total. Built to `AB_CARD_BASELINE.md`, same component pattern as the other two overlays.
- 2026-09-20 — **Reviewed and approved by Malthe — "fine within the parameters."** Templates in `templates/` are final as of this entry. Rebuilt both overlay templates again same day: the "corrected" version below was colored right but still structurally generic (a boxed accent-bar card reused for both components; the pipeline was an icon/badge/connector-line stepper, the single most default way to draw "automation flow" regardless of brand). Counter is now a full-width burn-in band (the literal OVERLAYS/chyron spec — band + top rule, not a floating card) with a bar-chart matching `04_bar_watchtime.svg` instead of a sparkline. Pipeline is now a typographic ledger (no icons, no badges, no connecting line — state carried by color and a status word, same device the stat cards use for numbers) instead of a stepper.
...
templates/mistake-reveal.html       3-beat claim/strike/correction hook clip (open in a browser)
